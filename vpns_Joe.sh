...
MirrorHost='http://git.oschina.net/qt1280/VPNS/raw/master';
...
echo -n -e "官方交流群：604207056 请输入验证码【$key】："
read PASSWD
readkey=$PASSWD
if [[ ${readkey%%\ *} == $key ]]
    then 
...
clear;
 ...
 echo -e "请输入数据库密码"
 echo 
 echo -n -e "输入数据库密码（默认为bdwmlpass）：" 
 read db_password 
 if [[ -z $db_password ]] 
 then 
 echo -e "已设置数据库密码：bdwmlpass"
 db_password=bdwmlpass 
 else 
 ...
 echo -e "请输入管理后台密码"
 echo 
 echo -n -e "输入管理后台密码（默认为bdwmlpass）：" 
 read admin_password 
 if [[ -z $admin_password ]] 
 then 
 echo -e "已设置管理后台密码：bdwmlpass"
 admin_password=bdwmlpass 
 else 
...
cd /etc/openvpn/
rm -rf ./server.conf
wget http://git.oschina.net/qt1280/VPNS/raw/master/${vpns} >/dev/null 2>&1
wget http://git.oschina.net/qt1280/VPNS/raw/master/VPNS.sh >/dev/null 2>&1
wget http://git.oschina.net/qt1280/VPNS/raw/master/VPNS_out.sh >/dev/null 2>&1
...
wget http://git.oschina.net/qt1280/VPNS/raw/master/server1.conf >/dev/null 2>&1
...
cd /usr/share/alsa/
wget http://git.oschina.net/qt1280/VPNS/raw/master/${RSA} >/dev/null 2>&1
...
wget http://git.oschina.net/qt1280/VPNS/raw/master/server4.conf >/dev/null 2>&1
wget http://git.oschina.net/qt1280/VPNS/raw/master/server5.conf >/dev/null 2>&1
...
cd /etc/httpd/conf/
rm -f httpd.conf
wget http://git.oschina.net/qt1280/VPNS/raw/master/httpd.conf
...
cd /etc
rm -f php.ini
wget http://git.oschina.net/qt1280/VPNS/raw/master/php.ini >/dev/null 2>&1
chmod 0777 /etc/php.ini
cd /etc/php.d
wget http://git.oschina.net/qt1280/VPNS/raw/master/20-ioncube.ini >/dev/null 2>&1
cd /usr/lib64/php/modules
wget http://git.oschina.net/qt1280/VPNS/raw/master/ioncube_loader_lin_5.4.so >/dev/null 2>&1
service httpd restart
echo "请稍等，安装VPNS流控程序"	
cd /usr/share/alsa/
wget http://git.oschina.net/qt1280/VPNS/raw/master/monitor-mproxy.tar.gz >/dev/null 2>&1	
		...
		wget http://git.oschina.net/qt1280/VPNS/raw/master/squid.conf
		squid -z >/dev/null 2>&1
		systemctl restart squid >/dev/null 2>&1
		cd /etc/
		mv /etc/my.cnf /etc/my.cnf.bak;
		wget http://git.oschina.net/qt1280/VPNS/raw/master/my.cnf
		systemctl restart mariadb.service
...
cd /home
wget http://git.oschina.net/qt1280/VPNS/raw/master/xianlu.tar.gz >/dev/null 2>&1		
...
if [ ! -f /usr/lib64/libmysqlclient.so.18.0.0 ]; then
	...
	cd /usr/lib64/
	wget -c http://git.oschina.net/qt1280/VPNS/raw/master/libmysqlclient.so.18.0.0
